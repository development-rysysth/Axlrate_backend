...
async function runMigration() {
  ...
  try {
    ...
    const migrationPath = join(__dirname, '../db/postgres/migrations/009_add_suggested_competitors_column.sql');
    const migrationSQL = readFileSync(migrationPath, 'utf-8');
    
    console.log('Running migration: 009_add_suggested_competitors_column.sql');
    console.log('Adding suggested_competitors column to hotels table...');
